...
def generate_launch_description():
    ...
    models_path = os.path.join(get_package_share_directory(package_name),
                               'models')
    ...
    world_arg = DeclareLaunchArgument(
        ...
        description='Specify the world file for Gazebo (e.g., test_world.sdf)')
    ...
    z_arg = DeclareLaunchArgument('z',
                                  default_value='0.5',
                                  description='Initial Z position')

    roll_arg = DeclareLaunchArgument('R',
                                     default_value='0.0',
                                     description='Initial Roll')

    pitch_arg = DeclareLaunchArgument('P',
                                      default_value='0.0',
                                      description='Initial Pitch')

    yaw_arg = DeclareLaunchArgument('Y',
                                    default_value='0.0',
    ...
    gazebo_launch = IncludeLaunchDescription(gazebo_pkg_launch,
                                             launch_arguments={
                                                 'gz_args':
                                                 [f'-r -v 4 ', world_file],
                                                 'on_exit_shutdown':
    ...
    spawn_model_gazebo_node = Node(
        ...
        executable='create',
        arguments=[
            '-name', robot_name, '-string', robot_description, '-x', x, '-y',
            y, '-z', z, '-R', roll, '-P', pitch, '-Y', yaw, '-allow_renaming',
            'false'
        ],
        output='screen',
    )
